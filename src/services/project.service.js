...
const ProjectService = {
  get: async (params) => {
    ...
    await RedisClient.set(
      projectCacheKey.replace(":code", resData.code),
      resData
    );
  ...
  search: async (params) => {
    ...
    if (user_code)
      docQuery = query(docQuery, where("user_code", "==", user_code));
    ...
    if (members && members.length > 0) {
      docQuery = query(
        docQuery,
        where("member_codes", "array-contains-any", members)
      );
    }
    ...
    if (teams && teams.length > 0) {
      docQuery = query(
        docQuery,
        where("team_codes", "array-contains-any", teams)
      );
    }
...
const ProjectMemberService = {
  search: async (params) => {
    ...
    const cacheKey = projectCacheKey.replace(":code", key);
    let entData = await RedisClient.get(cacheKey);
